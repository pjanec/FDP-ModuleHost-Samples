...
namespace ModuleHost.Network.Cyclone.Tests.Integration
{
    public class MultiInstanceScenarios
    {
        ...
        public void Scenario_MultiTurretTank_ReplicatesAcrossNodes()
        {
            ...
            var spawner2 = new NetworkSpawnerSystem(tkb2, elm2, strategy2, 1);
            
            // Node 2 receives EntityMaster
            var networkIdToEntity2 = new Dictionary<long, Entity>();
            var masterTranslator = new EntityMasterTranslator(new NodeIdMapper(1, 1), new TypeIdMapper(), 1, networkIdToEntity2);
            
            var masterMsg = new EntityMasterTopic { EntityId = 100, OwnerId = new NetworkAppId { AppDomainId = 1, AppInstanceId = 0 }, DisTypeValue = new DISEntityType { Kind = 1, Category = 1 }.Value };
            ...
            Assert.Equal(EntityLifecycle.Constructing, repo2.GetHeader(entity2.Index).LifecycleState);
            ...
            Assert.Equal(2, nw2.PrimaryOwnerId); // Primary owner is Node 1 (Mapped to 2)
            Assert.Equal(1, nw2.LocalNodeId);
        ...
        private class DeterministicOwnershipStrategy : IOwnershipDistributionStrategy
        {
            public int? GetInitialOwner(long descriptorTypeId, DISEntityType entityType, int masterNodeId, long instanceId)
            {
                if (descriptorTypeId == NetworkConstants.WEAPON_STATE_DESCRIPTOR_ID)
                {
                    // Instance 0 -> Master
                    // Instance 1 -> Peer
                    return instanceId == 0 ? masterNodeId : (masterNodeId == 1 ? 2 : 1);
                }
                return null;
            }
        }
