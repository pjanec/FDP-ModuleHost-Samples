...
namespace ModuleHost.Network.Cyclone.Systems
{
    ...
    {
        private readonly Fdp.Interfaces.IDataWriter _masterWriter;
        private readonly Dictionary<long, Entity> _trackedEntities = new();
        
        public CycloneNetworkCleanupSystem(Fdp.Interfaces.IDataWriter masterWriter)
        {
            _masterWriter = masterWriter;
        }
        ...
        public void Execute(ISimulationView view, float dt)
        {
            ...
            if (toRemove != null)
            {
                foreach (var netId in toRemove)
                {
                    ...
                    try 
                    {
                        // Use explicit destruction message
                        var deathNote = new EntityMasterTopic 
                        {
                             EntityId = netId,
                             Flags = 0xDEAD
                        };
                        _masterWriter.Write(deathNote);
                    }
                    catch (Exception ex)
                    {
